...
class single_train:
    ...
    def __init__(self, begin, end, is_DoS, DoS_begin, DoS_end, DoS_block, siding, block):
        ...
        duration = self.begin_ticks
        def get_sum_and_cur_block(number):
            # if (distance > block_begin), block go further; if (distance < block_end), block go close.
            while not (self.sum_block_dis[number] - self.block[self.cur_block[number]]) < self.distance[number] <= (
            self.sum_block_dis[number]):
        ...
        while duration < self.end_ticks:
            ...
            self.number, rank, self.one_schedule, self.all_schedule, n, self.refresh, self.time, self.is_DoS, self.DoS_begin_ticks, self.DoS_end_ticks, self.DoS_block, self.cur_block, self.distance, self.sum_block_dis, self.block, self.speed, self.siding, self.pos, self.ncolor, self.labels, self.pos_labels, self.T, headway = innerloop(self.number, rank, self.one_schedule, self.all_schedule, n, self.refresh, self.time, self.is_DoS, self.DoS_begin_ticks, self.DoS_end_ticks, self.DoS_block, self.cur_block, self.distance, self.sum_block_dis, self.block, self.speed, self.siding, self.pos, self.ncolor, self.labels, self.pos_labels, self.T, headway)
            ...
            #                     self.distance[rank[x]] = self.sum_block_dis[rank[x]] - self.block[
            #                         self.cur_block[rank[x]]]
...
start = time.clock()
single_train('2018-01-01 00:00:00', '2018-01-10 00:00:00', True, '2018-01-01 07:00:00', '2018-01-01 09:30:00', 23, [20, 30, 40, 60, 80, 100], [20] * 5000)
print('Used cython takes {}'.format(time.clock() - start))