...
class Track(Observable):
    def __init__(self, length, allow_sp, signalType):
        ...
        self.type = 'track'
        self.length = length
        self.entry_signal_L = AutoSignal(facing_direction='left') if signalType == 'abs' else HomeSignal(facing_direction='left')
        self.entry_signal_R = AutoSignal(facing_direction='right') if signalType == 'abs' else HomeSignal(facing_direction='right')
        self.train = None
